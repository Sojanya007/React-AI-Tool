/*function RecentSearch({ recentHistory, setRecentHistory, setSelectedHistory, showHistory, setShowHistory, darkMode, setDarkMode }) {
    ...
    return (
        <>
            <div className={`col-span-1 dark:bg-zinc-800 bg-red-100 pt-3 w-64 h-screen overflow-visible fixed top-0 left-0 z-40 ${showHistory ? 'flex' : 'hidden'} md:static md:block md:w-full md:h-screen md:col-span-1  flex flex-col`}>
                ...
                <ul className='text-left overflow-y-auto overflow-x-hidden mt-2 flex-1 min-h-0 [&::-webkit-scrollbar]:hidden  scrollbar-none'>
                    {
                        recentHistory && recentHistory.map((item, index) => (
                            <li onClick={() => { setSelectedHistory(item); setShowHistory(false); }} key={index} className='pl-3 px-5 truncate dark:text-zinc-400 text-zinc-700 cursor-pointer dark:hover:bg-zinc-700 dark:hover:text-zinc-200 hover:bg-red-200 hover:text-zinc-800'>
                                {item}
                            </li>
                        ))
                    }
                ...
                <div className="shrink-0 p-3 relative z-50">
                    <select value={darkMode} onChange={(event) => setDarkMode(event.target.value)} className='w-full p-2 rounded dark:bg-zinc-700 bg-white dark:text-white text-zinc-800 outline-none'>
